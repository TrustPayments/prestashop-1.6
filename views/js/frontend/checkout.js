...
jQuery(function ($) {
    ...
    var trustpayments_checkout = {
        ...
        show_processing_spinner : function () {
            $('#trustpayments-processing-spinner-container').removeClass('invisible');
        },
    
        hide_processing_spinner: function () {
            $('#trustpayments-processing-spinner-container').addClass('invisible');
        },
        ...
        remove_existing_errors : function () {
            $('#trustpayments-error-messages').empty();
            $('#trustpayments-error-messages').removeClass("alert alert-danger");
        },
    
        show_new_errors : function (message) {
            $('#trustpayments-error-messages').addClass('alert alert-danger');
            $('#trustpayments-error-messages').append('<div>'+message+'</div>');
            $('html, body').animate({
                scrollTop : ($('#trustpayments-error-messages').offset().top - 20)
            }, 1000);
        },
