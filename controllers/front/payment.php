...
class TrustPaymentsPaymentModuleFrontController extends TrustPaymentsFrontpaymentcontroller
{
    ...
    public function initContent()
    {
        ...
        $cartHash = TrustPaymentsHelper::calculateCartHash($cart);
        $showCart = Configuration::get(TrustPayments::CK_SHOW_CART, null, null, $cart->id_shop);
        $showTos = Configuration::get(TrustPayments::CK_SHOW_TOS, null, null, $cart->id_shop);

        $jsUrl = null;
        try {
            $jsUrl = TrustPaymentsServiceTransaction::instance()->getJavascriptUrl($cart);
        } catch (Exception $e) {
            $this->context->cookie->tru_error = $this->module->l(
                'There was a technical issue, please try again.',
                'payment'
        ...
        $this->context->smarty->registerPlugin(
            'function',
            'trustpayments_resolve_template',
            array(
                $this,
                'resolveTemplatePath'
            )
