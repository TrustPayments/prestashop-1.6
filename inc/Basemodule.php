 ...
 */
class TrustPaymentsBasemodule
{
    const CK_BASE_URL = 'TRU_BASE_GATEWAY_URL';

    const CK_USER_ID = 'TRU_USER_ID';

    const CK_APP_KEY = 'TRU_APP_KEY';

    const CK_SPACE_ID = 'TRU_SPACE_ID';

    const CK_SPACE_VIEW_ID = 'TRU_SPACE_VIEW_ID';

    const CK_MAIL = 'TRU_SHOP_EMAIL';

    const CK_CART_RECREATION = 'TRU_CART_RECREATION';

    const CK_INVOICE = 'TRU_INVOICE_DOWNLOAD';

    const CK_PACKING_SLIP = 'TRU_PACKING_SLIP_DOWNLOAD';

    const CK_LINE_ITEM_CONSISTENCY = 'TRU_LINE_ITEM_CONSISTENCY';

    const CK_FEE_ITEM = 'TRU_FEE_ITEM';

    const CK_SURCHARGE_ITEM = 'TRU_SURCHARGE_ITEM';

    const CK_SURCHARGE_TAX = 'TRU_SURCHARGE_TAX';

    const CK_SURCHARGE_AMOUNT = 'TRU_SURCHARGE_AMOUNT';

    const CK_SURCHARGE_TOTAL = 'TRU_SURCHARGE_TOTAL';

    const CK_SURCHARGE_BASE = 'TRU_SURCHARGE_BASE';

    const CK_STATUS_FAILED = 'TRU_STATUS_FAILED';

    const CK_STATUS_AUTHORIZED = 'TRU_STATUS_AUTHORIZED';

    const CK_STATUS_VOIDED = 'TRU_STATUS_VOIDED';

    const CK_STATUS_COMPLETED = 'TRU_STATUS_COMPLETED';

    const CK_STATUS_MANUAL = 'TRU_STATUS_MANUAL';

    const CK_STATUS_DECLINED = 'TRU_STATUS_DECLINED';

    const CK_STATUS_FULFILL = 'TRU_STATUS_FULFILL';
    ...
    const CK_RUN_LIMIT = 'TRU_RUN_LIMIT';
        
    private static $recordMailMessages = false;

    private static $recordedMailMessages = array();

    public static function install(TrustPayments $module)
    {
        ...
        if (! TrustPaymentsMigration::installDb()) {
            $module->addError(Tools::displayError('Unable to install database tables.'));
            return false;
        }
        TrustPaymentsOrderstatus::registerOrderStatus();
    ...
    public static function uninstall(TrustPayments $module)
    {
    ...
    public static function installHooks(TrustPayments $module)
    {
    ...
    public static function uninstallConfigurationValues()
    {
        return
            ...
            Configuration::deleteByName(TrustPaymentsServiceManualtask::CONFIG_KEY) &&
    ...
    private function installControllers(TrustPayments $module)
    {
    ...
    public static function addTab(TrustPayments $module, $className, $name, $parentId)
    {
        $tab = new Tab();
    ...
    private static function uninstallControllers(TrustPayments $module)
    {
        $result = true;
    ...
    public static function displayHelpButtons(TrustPayments $module)
    {
        return $module->display(dirname(dirname(__FILE__)), 'views/templates/admin/admin_help_buttons.tpl');
    }

    public static function getMailHookActiveWarning(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveAll(TrustPayments $module)
    {
        $output = "";
        if (Tools::isSubmit('submit' . $module->name . '_all')) {
            ...
            if ($refresh) {
                $error = Hook::exec('trustPaymentsSettingsChanged');
                if (! empty($error)) {
                    $output .= $module->displayError($error);
                }
            }
    ...
    public static function handleSaveApplication(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveCartRecreation(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveEmail(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveFeeItem(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveDownload(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveSpaceViewId(TrustPayments $module)
    {
        $output = "";
    ...
    public static function handleSaveOrderStatus(TrustPayments $module)
    {
        $output = "";
     ...
     * @return string
     */
    public static function handleSaveCronSettings(TrustPayments $module)
    {
        $output = "";
        if (Tools::isSubmit('submit' . $module->name . '_email')) {
            if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    private static function getFormHelper(TrustPayments $module)
    {
    ...
    public static function displayForm(TrustPayments $module)
    {
        $userIdConfig = array(
            'type' => 'text',
        ...
        $fieldsForm[]['form'] = array(
            'legend' => array(
                'title' => 'Trust Payments ' . $module->l('General Settings', 'basemodule')
            ),
            'input' => $generalInputs,
            'buttons' => $buttons
        );
    ...
    public static function getApplicationConfigValues(TrustPayments $module)
    {
        $values = array();
        if ($module->getContext()->shop->isFeatureActive()) {
            if ($module->getContext()->shop->getContext() == Shop::CONTEXT_ALL) {
    ...
    public static function getCartRecreationForm(TrustPayments $module)
    {
    ...
    public static function getEmailForm(TrustPayments $module)
    {
        $emailConfig = array(
            array(
                'type' => 'switch',
    ...
    public static function getCartRecreationConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function getEmailConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function getFeeForm(TrustPayments $module)
    {
    ...
    public static function getFeeItemConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function getDocumentForm(TrustPayments $module)
    {
    ...
    public static function getDownloadConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function getSpaceViewIdForm(TrustPayments $module)
    {
    ...
    public static function getSpaceViewIdConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
            $values[self::CK_SPACE_VIEW_ID] = Configuration::get(self::CK_SPACE_VIEW_ID);
    ...
    public static function getOrderStatusForm(TrustPayments $module)
    {
    ...
    public static function getOrderStatusConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
     ...
     * @return mixed[]
     */
    public static function getCronSettingsForm(TrustPayments $module) {
        $cronSettings = array(
            array(
                'type' => 'text',
     ...
     * @return mixed[]
     */
    public static function getCronSettingsConfigValues(TrustPayments $module)
    {
        $values = array();
        if (! $module->getContext()->shop->isFeatureActive() || $module->getContext()->shop->getContext() == Shop::CONTEXT_SHOP) {
    ...
    public static function hookTrustPaymentsSettingsChanged(TrustPayments $module, $params)
    {
        try {
            TrustPaymentsHelper::resetApiClient();
            TrustPaymentsHelper::getApiClient();
        } catch (TrustPaymentsExceptionIncompleteconfig $e) {
            // We stop here as the configuration is not complete
            return "";
        }
        $errors = array();
        try {
            TrustPaymentsServiceMethodconfiguration::instance()->synchronize();
        } catch (Exception $e) {
        ...
        try {
            TrustPaymentsServiceWebhook::instance()->install();
        } catch (Exception $e) {
            PrestaShopLogger::addLog($e->getMessage(), 2, null, null, false);
            $errors[] = $module->l(
        ...
        try {
            TrustPaymentsServiceManualtask::instance()->update();
        } catch (Exception $e) {
            PrestaShopLogger::addLog($e->getMessage(), 2, null, null, false);
    ...
    {
        $toDelete = array(
            'trustpayments_currencies',
            'trustpayments_label_description',
            'trustpayments_label_description_group',
            'trustpayments_languages',
            'trustpayments_connectors',
            'trustpayments_methods'
        );
    ...
    public static function getParametersFromMethodConfiguration(
        ...
        $translatedName = TrustPaymentsHelper::translate($methodConfiguration->getTitle(), $language);
        ...
        if (! empty($img) && $methodConfiguration->isShowImage()) {
            $parameters['image'] = TrustPaymentsHelper::getResourceUrl(
                $methodConfiguration->getImageBase(),
                $methodConfiguration->getImage(),
                TrustPaymentsHelper::convertLanguageIdToIETF($cart->id_lang),
                $spaceId,
                $spaceViewId
            );
        }
        $parameters['description'] = '';
        $description = TrustPaymentsHelper::translate($methodConfiguration->getDescription(), $language);
        ...
        $surchargeValues = TrustPaymentsFeehelper::getSurchargeValues($cart);
        if ($surchargeValues['surcharge_total'] > 0) {
            $parameters['surchargeValues'] = $surchargeValues;
        } else {
            $parameters['surchargeValues'] = array();
        }
        $feeValues = TrustPaymentsFeehelper::getFeeValues($cart, $methodConfiguration);
        ...
        if ($module->active) {
            ...
            try {
                ...
                if ($isCartRecreation) {
                    ...
                    // again
                    TrustPaymentsServiceTransaction::instance()->checkTransactionPending($originalCart);
                ...
                if (strpos($payment_method, "trustpayments_") === 0) {
                    $id = Tools::substr($payment_method, strpos($payment_method, "_") + 1);
                    $methodConfiguration = new TrustPaymentsModelMethodconfiguration($id);
                }

                if ($methodConfiguration == null || $methodConfiguration->getId() == null ||
                    $methodConfiguration->getState() != TrustPaymentsModelMethodconfiguration::STATE_ACTIVE || $methodConfiguration->getSpaceId() !=
                ...
                $translatedTitel = TrustPaymentsHelper::translate(
                ...
                foreach ($orders as $order) {
                    TrustPaymentsHelper::updateOrderMeta(
                        $order,
                        'trustPaymentsMethodId',
                        $methodConfiguration->getId()
                    );
                    TrustPaymentsHelper::updateOrderMeta(
                        $order,
                        'trustPaymentsMainOrderId',
                        $dataOrder->id
                    );
                    $order->save();
                }
                $emailMessages = TrustPaymentsBasemodule::stopRecordingMailMessages();

                // Update cart <-> Trust Payments mapping <-> order mapping
                $ids = TrustPaymentsHelper::getCartMeta($originalCart, 'mappingIds');
                TrustPaymentsHelper::updateOrderMeta($dataOrder, 'mappingIds', $ids);
                if (Configuration::get(self::CK_MAIL, null, null, $cart->id_shop)) {
                    TrustPaymentsHelper::storeOrderEmails($dataOrder, $emailMessages);
                }
                TrustPaymentsHelper::updateOrderMeta($dataOrder, 'originalCart', $originalCart->id);
            ...
            try {
                $transaction = TrustPaymentsServiceTransaction::instance()->confirmTransaction(
                    $dataOrder,
                    $orders,
                    $methodConfiguration->getConfigurationId()
                );
                TrustPaymentsServiceTransaction::instance()->updateTransactionInfo($transaction, $dataOrder);
                $GLOBALS['trustpaymentsTransactionIds'] = array(
                    'spaceId' => $transaction->getLinkedSpaceId(),
                    'transactionId' => $transaction->getId()
                );
            } catch (Exception $e) {
                PrestaShopLogger::addLog($e->getMessage(), 3, null, null, false);
                TrustPaymentsHelper::deleteOrderEmails($dataOrder);
                TrustPaymentsBasemodule::startRecordingMailMessages();
    ...
    public static function hookDisplayOrderDetail(TrustPayments $module, $params)
    {
        $order = $params['order'];
        ...
        if (in_array(
            ...
            $documentVars['trustPaymentsInvoice'] = $module->getContext()->link->getModuleLink(
                'trustpayments',
                'documents',
                array(
                    'type' => 'invoice',
        ...
        if ($transactionInfo->getState() == \TrustPayments\Sdk\Model\TransactionState::FULFILL &&
            (bool) Configuration::get(self::CK_PACKING_SLIP)) {
            $documentVars['trustPaymentsPackingSlip'] = $module->getContext()->link->getModuleLink(
                'trustpayments',
                'documents',
                array(
                    'type' => 'packingSlip',
    ...
    public function hookActionOrderGridQueryBuilderModifier(TrustPayments $module, $params) {
        ...
        $searchQueryBuilder->leftJoin(
            'o',
            '`' . pSQL(_DB_PREFIX_) . 'tru_transaction_info`',
            'wtransinfo',
            'wtransinfo.`order_id` = o.`id_order`'
        );
    }

    public function hookActionOrderGridDefinitionModifier(TrustPayments $module, $params) {
        ...
        $actionsCollection->add(
            (new LinkRowAction('download_invoice'))
                ->setName("Download TrustPayments Invoice")
                ->setIcon('description')
                ->setOptions([
    ...
    public static function hookActionAdminControllerSetMedia(TrustPayments $module, $arr)
    {
        if (Tools::strtolower(Tools::getValue('controller')) == 'adminorders') {
            Media::addJsDefL('trustpayments_admin_token', $module->getContext()->link->getAdminLink('AdminTrustPaymentsDocuments'));
    ...
    public static function hookDisplayBackOfficeHeader(TrustPayments $module, $params)
    {
        if (Module::isEnabled($module->name)) {
            try {
                TrustPaymentsMigration::migrateDb();
            } catch (Exception $e) {
                $module->displayError(
                    $module->l(
                        sprintf(
                ...
                PrestaShopLogger::addLog($e->getMessage(), 3, null, 'TrustPayments');
            }
        }
    ...
    private static function handleVoucherDeleteRequest(TrustPayments $module)
    {
    ...
    private static function handleRefundRequest(TrustPayments $module)
    {
    ...
    private static function handleCancelProductRequest(TrustPayments $module)
    {
        if (array_key_exists('cancelProduct', $_POST)) {
            ...
            $strategy = TrustPaymentsBackendStrategyprovider::getStrategy();
            if ($strategy->isVoucherOnlyTrustPayments($order, $cancelParameters)) {
                return;
            }
            ...
            if ($module->hasBackendControllerDeleteAccess($backendController)) {
                ...
                if ($strategy->isCancelRequest($order, $cancelParameters)) {
                    ...
                    try {
                        ...
                        $backendController->errors[] = TrustPaymentsHelper::cleanExceptionMessage(
                            $e->getMessage()
                        );
                    }
                }
     ...
     * @return string
     */
    public static function hookDisplayAdminAfterHeader(TrustPayments $module)
    {
        $manualTasks = TrustPaymentsServiceManualtask::instance()->getNumberOfManualTasks();
        $url = TrustPaymentsHelper::getBaseGatewayUrl();
    ...
    public static function hookDisplayAdminOrderTabOrder(TrustPayments $module, $params)
    {
        ...
        $templateVars = array();
        $templateVars['trustPaymentsDocumentsCount'] = 0;
        if (in_array(
            $transactionInfo->getState(),
            array(
                \TrustPayments\Sdk\Model\TransactionState::COMPLETED,
            ...
            $templateVars['trustPaymentsDocumentsCount'] ++;
        }
        if ($transactionInfo->getState() == \TrustPayments\Sdk\Model\TransactionState::FULFILL) {
            $templateVars['trustPaymentsDocumentsCount'] ++;
        }
    ...
    public static function hookDisplayAdminOrderContentOrder(TrustPayments $module, $params)
    {
        ...
        $templateVars = array();
        $templateVars['trustPaymentsDocumentsCount'] = 0;
        $templateVars['trustPaymentsDocuments'] = array();
        if (in_array(
            $transactionInfo->getState(),
            array(
                \TrustPayments\Sdk\Model\TransactionState::COMPLETED,
            ...
            $templateVars['trustPaymentsDocuments'][] = array(
                ...
                'url' => $module->getContext()->link->getAdminLink('AdminTrustPaymentsDocuments') .
                '&action=trustPaymentsInvoice&id_order=' . $order->id
            );
            $templateVars['trustPaymentsDocumentsCount'] ++;
        }
        if ($transactionInfo->getState() == \TrustPayments\Sdk\Model\TransactionState::FULFILL) {
            $templateVars['trustPaymentsDocuments'][] = array(
                ...
                'url' => $module->getContext()->link->getAdminLink('AdminTrustPaymentsDocuments') .
                '&action=trustPaymentsPackingSlip&id_order=' . $order->id
            );
            $templateVars['trustPaymentsDocumentsCount'] ++;
        }
        $module->getContext()->smarty->assign($templateVars);
    ...
    public static function hookDisplayAdminOrder(TrustPayments $module, $params)
    {
        $orderId = $params['id_order'];
        $order = new Order($orderId);
        $transactionInfo = TrustPaymentsHelper::getTransactionInfoForOrder($order);
        ...
        $templateVars = array();
        $templateVars['isTrustPaymentsTransaction'] = true;
        if ($transactionInfo->getState() == \TrustPayments\Sdk\Model\TransactionState::AUTHORIZED) {
            if (! TrustPaymentsModelCompletionjob::isCompletionRunningForTransaction(
                $transactionInfo->getSpaceId(),
                $transactionInfo->getTransactionId()
            ) && ! TrustPaymentsModelVoidjob::isVoidRunningForTransaction(
                $transactionInfo->getSpaceId(),
                $transactionInfo->getTransactionId()
            )) {
                ...
                $templateVars['voidUrl'] = $module->getContext()->link->getAdminLink('AdminTrustPaymentsOrder', true) .
                    '&action=voidOrder&ajax=1&id_order=' . $orderId;
                $templateVars['completionUrl'] = $module->getContext()->link->getAdminLink(
                    'AdminTrustPaymentsOrder',
                    true
        ...
        if ($transactionInfo->getState() == \TrustPayments\Sdk\Model\TransactionState::VOIDED) {
            $templateVars['editButtons'] = true;
        ...
        if (TrustPaymentsModelVoidjob::isVoidRunningForTransaction(
            $transactionInfo->getSpaceId(),
            $transactionInfo->getTransactionId()
        )) {
            $templateVars['voidPending'] = true;
        }
        if (TrustPaymentsModelRefundjob::isRefundRunningForTransaction(
            $transactionInfo->getSpaceId(),
            $transactionInfo->getTransactionId()
        )) {
        ...
        $module->getContext()->smarty->assign($templateVars);
        TrustPaymentsVersionadapter::getAdminOrderTemplate();
    ...
    {
        // We need to start a db transaction here to revert changes to the order, if the update to Trust Payments fails.
    ...
    public static function hookActionObjectOrderPaymentAddBefore(TrustPayments $module, $params)
    {
    ...
    public static function hookActionOrderEdited(TrustPayments $module, $params)
    {
        // We send the changed line items to Trust Payments after the order has been edited
        $action = Tools::getValue('action');
        if (in_array($action, array(
            'editProductOnOrder',
            'deleteProductLine',
            ...
            $lineItems = TrustPaymentsServiceLineitem::instance()->getItemsFromOrders($orders);
            $transactionInfo = TrustPaymentsHelper::getTransactionInfoForOrder($modifiedOrder);
            if (! $transactionInfo) {
                TrustPaymentsHelper::rollbackDBTransaction();
                die(
                    Tools::jsonEncode(
    ...
    public static function hookDisplayTop(TrustPayments $module, $params)
    {
        return self::getCronJobItem($module);
    }
    
    public static function getCronJobItem(TrustPayments $module)
    {
        TrustPaymentsCron::cleanUpHangingCrons();
        TrustPaymentsCron::insertNewPendingCron();
        
        $currentToken = TrustPaymentsCron::getCurrentSecurityTokenForPendingCron();
        if ($currentToken) {
    ...
    public static function hookTrustPaymentsCron($params)
    {
        $tasks = array();
        $tasks[] = 'TrustPaymentsCron::cleanUpCronDB';
        $voidService = TrustPaymentsServiceTransactionvoid::instance();
        ...
        $completionService = TrustPaymentsServiceTransactioncompletion::instance();
        ...
        $refundService = TrustPaymentsServiceRefund::instance();
