 ...
 */
class TrustPaymentsBackendDefaultstrategy implements TrustPaymentsBackendIstrategy
{
    ...
    public function validateAndParseData(Order $order, array $postData)
    {
        ...
        throw new Exception(
            TrustPaymentsHelper::getModuleInstance()->l('The refund type is not supported.', 'defaultstrategy')
        );
    }
    ...
    public function simplifiedRefund(array $postData)
    {
        ...
        if ($amount >= 0) {
            return array(
                ...
                'trustPaymentsOffline' => isset($postData['trustpayments_offline'])
            );
        } else {
            if (! empty($quantity)) {
                throw new Exception(
    ...
    private function validateDataCancelProductType(Order $order, array $postData)
    {
        if (! isset($postData['id_order_detail']) && ! isset($postData['id_customization'])) {
            throw new Exception(
                TrustPaymentsHelper::getModuleInstance()->l('You must select a product.', 'defaultstrategy')
            );
        } elseif (! isset($postData['cancelQuantity']) && ! isset($postData['cancelCustomizationQuantity'])) {
            throw new Exception(
                TrustPaymentsHelper::getModuleInstance()->l('You must enter a quantity.', 'defaultstrategy')
            );
        } else {
    ...
    private function getUsedDiscountUniqueIds(
        ...
        $currencyCode = TrustPaymentsHelper::convertCurrencyIdToCode($order->id_currency);
        // Discount Rate
        if ($reductionPercent > 0) {
            if ($reductionProduct > 0) {
                ...
                if ($reductionProduct == - 2) {
                    $selectedProducts = TrustPaymentsCartruleaccessor::checkProductRestrictionsStatic(
                        $cartRule,
                ...
                foreach ($usedTaxes as $id => $values) {
                    ...
                    $totalAmount = TrustPaymentsHelper::roundAmount(
        ...
        if ((float) $reductionAmount > 0) {
            if ($reductionProduct > 0) {
                ...
                foreach ($usedTaxes as $id => $values) {
                    ...
                    $totalAmount = TrustPaymentsHelper::roundAmount($amount * - 1, $currencyCode);
                    if ($totalAmount == 0) {
                        continue;
                    }
                    $discountUniqueIds[] = $uniqueIdBase . '-' . $id;
    ...
    private function applyRefundPartialRefundType(Order $order, array $parsedData)
    {
        ...
        if ($parsedData['generateDiscountRefund']) {
            ...
            $cart_rule->description = sprintf(
                TrustPaymentsHelper::getModuleInstance()->l('Credit slip for order #%d', 'defaultstrategy'),
                $order->id
            );
    ...
    public function afterApplyRefundActions(Order $order, array $parsedData, array $appliedData)
    {
        ...
        if ($parsedData['refundType'] == self::REFUND_TYPE_PARTIAL_REFUND ||
            ...
            if (Configuration::get(TrustPaymentsBasemodule::CK_MAIL, null, null, $order->id_shop)) {
                $params = array();
    ...
    {
        if ($parsedData['trustPaymentsOffline']) {
            return \TrustPayments\Sdk\Model\RefundType::MERCHANT_INITIATED_OFFLINE;
        }
        return \TrustPayments\Sdk\Model\RefundType::MERCHANT_INITIATED_ONLINE;
    }
    ...
    public function processVoucherDeleteRequest(Order $order, array $data)
    {
        ...
        if (Validate::isLoadedObject($order_cart_rule) && $order_cart_rule->id_order == $order->id) {
            ...
            throw new Exception(
                TrustPaymentsHelper::getModuleInstance()->l('You cannot edit this cart rule.', 'defaultstrategy')
            );
        }
    }
