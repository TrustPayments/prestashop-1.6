...
class TrustPaymentsFrontpaymentcontroller extends ModuleFrontController
{
    ...
    protected function checkAvailablility(Cart $cart)
    {
        if ($cart->id_address_delivery == 0 || $cart->id_address_invoice == 0 || ! $this->module->active || ! Validate::isLoadedObject(new Customer($cart->id_customer))) {
            $this->context->cookie->tru_error = $this->module->l('Your session expired, please try again.', 'frontpaymentcontroller');
        ...
        foreach (Module::getPaymentModules() as $module) {
            if ($module['name'] == 'trustpayments') {
                $authorized = true;
                break;
            }
        }
        if (! $authorized) {
            $this->context->cookie->tru_error = $this->module->l('This payment method is no longer available, please try another one.', 'frontpaymentcontroller');
            return $this->context->link->getPageLink('order', true, null, "step=3");
        }
        if (! $this->module instanceof TrustPayments) {
            $this->context->cookie->tru_error = $this->module->l('There was a technical issue, please try again.', 'frontpaymentcontroller');
            return $this->context->link->getPageLink('order', true, null, "step=3");
        }
        return null;
    }
